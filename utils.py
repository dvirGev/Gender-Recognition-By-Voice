...
def load_data(vector_length=705):
    ...
    for i, (filename, gender) in tqdm.tqdm(enumerate(zip(df['filename'], df['gender'])), "Loading data", total=n_samples):
        features = extract_feature(filename, mel=True, chroma=True, contrast=True, mfcc=True, tonnetz=True).reshape(1, -1)
        X[i] = np.append(features, xVector(filename), axis=None)
